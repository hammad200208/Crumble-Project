...
const Card = () => {
  ...
  const [selectedIndex, setSelectedIndex] = useState(1); // Default: card2
  ...
  return (
    <div className="relative py-10 px-4 sm:px-10 lg:px-20">
      ...
      <div
        ...
        className="z-20 absolute top-1/2  transform -translate-y-1/2 bg-[#000996] rounded-full shadow-md p-5 cursor-pointer hover:scale-105 transition"
      >
      ...
      <div
        ...
        className="z-20 absolute top-1/2 right-20 transform -translate-y-1/2 bg-[#000996] rounded-full shadow-md p-5 cursor-pointer hover:scale-105 transition"
      >
        <AiOutlineRight className="text-white w-6 h-6" />
      </div>

      {/* Cards */}
      <div className="absolute top-[30%] left-1/2 -translate-x-1/2 w-full max-w-6xl flex flex-wrap justify-center gap-6 px-4 z-10">
        {products.map((product, index) => (
          <div
            key={index}
            className={`bg-white rounded-2xl shadow-lg w-60 pt-16 pb-6 px-4 flex flex-col items-center text-center relative transition-all duration-300
              ${
                selectedIndex === index
